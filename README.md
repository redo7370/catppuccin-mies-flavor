...
This ruleset by default makes all windows slightly transparent and forces windows to apply the themes buttons even if they draw their own. If you open an app or game that has this opacity applied, you'll have to **exclude** it manually. This is possible by opening the window, pressing <kbd>Alt</kbd>+<kbd>F3</kbd>. A small drop-down menu should open. Click on **More Actions → Configure Special Application Settings**. A separate window will open which targets only the window you had active when pressing <kbd>Alt</kbd>+<kbd>F3</kbd>. Here you can click on **Add Property** and choose **Active Opacity** and **Inactive Opacity** to adjust them to your liking. Set both to **100%** if you want them to be solid. For **No Title Bar and Frame** if the window draws their own, add the property **No title bar and frame** and adjust it to your liking. Choosing `Force` and `yes` will always make the window use only its self-drawn buttons.s
